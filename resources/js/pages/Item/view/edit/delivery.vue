...
export default {
	...
	methods: {
		  async handleSubmit() {
			...
			this.form.reversal.remarks= (('').concat(' ',(res.data.remarks))).replace("null","").trim();
				...
				for (let i = 0; i < res.data.hist.length; i++) {
					this.form.items.push( 
						...
						bal: parseInt(res.data.hist[i].bal)+parseInt(res.data.hist[i].drQtyCase*res.data.hist[i].numperuompu),
						tins: (res.data.hist[i].unit=="CASE")?res.data.hist[i].drQtyCase*res.data.hist[i].numperuompu:res.data.hist[i].drQtyCase,
