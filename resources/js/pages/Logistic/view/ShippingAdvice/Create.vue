			...
			<div class="card-body">
				...
				<div>
					...
					<div class="tab-pane" v-show="current_step == 3">
						<div class="form-group mb-0" v-if="!drYesNo">
...
export default {
	...
	middleware: "auth",
	metaInfo() {
		return { title: "Shipping Advice - Create" };
	},
	...
	methods: {
        getLook(){
            console.log('aa');
            return this.getLookup('LGDR').fulltitle;
        },
		handleEnter() {
			(this.formImport.trnmode = "arinvoiceDR"),
				this.formImport.post("/api/process/logistic").then((res) => {
                    this.drDataCustomer = res.data[0]['Customer']
		...
		handleAddA() {
            var data = this.drData;
            for (let index = 0; index < this.drData.length; index++) { 

                this.form.items.push({
                    itemcode:this.drData[index]['itemcode'],
                    itemdesc:this.drData[index]['Description'],
                    trntype: "SHA",
                    qty:this.formatNumberD(this.drData[index]['Qty']*1,0),
                    unit:this.drData[index]['Unit'],
                    price:this.formatNumberDis(this.drData[index]['Unit_price']),
                    // itemdesc:this.drData[index]['Unit_price'],
                    total: this.formatNumberDis((this.drData[index]['Qty']*1) * (this.drData[index]['Unit_price'])),
                })  
            }
		...
		advanceStep() {
            var val = this.getLookup('LOGS');
            if(val[0]){
                if(val[0]['fulltitle']=='YES'){
                    this.drYesNo = true;
                } 
            }
			if (!this.validate()) {
